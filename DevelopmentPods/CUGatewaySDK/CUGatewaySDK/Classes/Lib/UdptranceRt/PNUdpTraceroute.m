...
@property (nonatomic,copy) PNUdpTracerouterIpArrHandler completeIpArr;
@property (nonatomic,strong) NSMutableString *traceDetails;

@property (nonatomic, strong)NSMutableArray *traceIPArr;
...
- (instancetype)init:(NSString *)host
              maxTtl:(NSUInteger)maxTtl
            completeIpArr:(PNUdpTracerouterIpArrHandler)complete
{
    if (self = [super init]) {
        _host = host == nil ? @"" : host;
        _maxTtl = maxTtl;
        _completeIpArr = complete;
        _isStop = NO;
    }
    return self;
}

- (NSMutableArray *)traceIPArr{
    if (!_traceIPArr) {
        _traceIPArr = [NSMutableArray new];
    }
    return  _traceIPArr;
}
...
- (void)sendAndRec
{
    ...
    [self.traceIPArr removeAllObjects];
    ...
    do {
        ...
        if (trace.routeIp == nil) {
            ...
            [self.traceIPArr addObject:trace.routeIp];
...
//    NSLog(@"结束：%@",self.traceIPArr);
    [_traceDetails appendString:@"udp traceroute complete...\n"];
    if (_complete) {
        _complete(_traceDetails);
    }
    if (_completeIpArr) {
        _completeIpArr(_traceIPArr);
    }
...
+ (instancetype)startAndCompleteIparr:(PNUdpTracerouterIpArrHandler _Nonnull)complete{
    return  [PNUdpTraceroute start:@"114.114.114.114" maxTtl:5 completeIparr:complete];
}

+ (instancetype)start:(NSString *_Nonnull)host maxTtl:(NSUInteger)maxTtl completeIparr:(PNUdpTracerouterIpArrHandler _Nonnull)complete{
    PNUdpTraceroute *udpTrace  = [[PNUdpTraceroute alloc] init:host maxTtl:maxTtl completeIpArr:complete];
    [PNetQueue pnet_async:^{
        [udpTrace sendAndRec];
    }];
    return udpTrace;
